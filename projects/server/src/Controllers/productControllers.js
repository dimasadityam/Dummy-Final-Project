...
module.exports = {
  getProduct: async (req, res, next) => {
    try {
      ...
      if (req.body.idProducts) {
        ...
        let getProducts = await dbQuery(`SELECT p.idProduct, p.idCategory, p.productName, p.productPicture, p.defaultUnit, p.convertedQuantity,
  ...
  getAllProduct: async (req, res, next) => {
    try {
      ...
      let getAllProducts = await dbQuery(`SELECT p.idProduct, p.idCategory, p.productName, p.productPicture, p.defaultUnit, p.convertedQuantity,
  ...
  konversiStock: async (req, res, next) => {
    try {
      ...
      console.log("req.dataUser", req.dataUser)
      if (req.body.idProducts) {
        let updateStock = req.body.stocksQuantityMain -= req.body.conversionQty
        ...
        let konversi = await dbQuery(`UPDATE stocks SET stockQuantity = ${updateStock} WHERE idStock = ${req.body.idStocksMain};`)
        let updateKonversi = await dbQuery(`UPDATE stocks SET stockQuantity = ${updateConversion} WHERE idStock = ${req.body.idStocks};`)
        let updateProductHistoryMain = await dbQuery(`INSERT INTO producthistory (idStock, idTransaction, mutationSource, mutationType)
          VALUES (${req.body.idStocksMain}, 0, 'Unit conversion', 'Pengurangan');`)
        let updateProductHistory = await dbQuery(`INSERT INTO producthistory (idStock, idTransaction, mutationSource, mutationType)
