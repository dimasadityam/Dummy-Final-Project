...
module.exports = {
  ...
  //   try {
  //     let emailLogin = await dbQuery(`SELECT * FROM users WHERE email LIKE '%${req.body.email}%' and password LIKE '%${hashPassword(req.body.password)}%';`);
  //     // let checkEmail = await dbQuery(`SELECT email FROM users WHERE email = '${req.body.email}'; `)
  //     // let checkPassword = await dbQuery(`SELECT password FROM users WHERE password = '${hashPassword(req.body.password)}'; `)

  //     console.log(hashPassword(req.body.password))
  //     console.log("emailLogin", emailLogin[0])
  //     if (req.body.email != emailLogin[0].email) {
  //       let message = "Email Anda Salah."
  //       return res.status(404).send({
  //         message
  //       });
  //     } else if (emailLogin[0].password != hashPassword(req.body.password)) {
  //       let message = "Password Anda Salah."
  //       return res.status(404).send({
  //         message
  //       });
  //     } else {
  ...
  //         let { idUser, name, email, password, role, phone, profilePicture, addDate, isVerified } = emailLogin[0]
