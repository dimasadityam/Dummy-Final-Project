...
import { getProducts, getProductActions } from "../../Redux/Actions/productActions";
...
function ModalConversion(props) {
  ...
  const getProducts =async() => {
      try {
        ...
        if (token) {
          let res = await Axios.post(`${API_URL}/product/getProduct`,{
            idProducts: 3
          },{
            headers: {
  ...
  const handleKonversi =async() => {
      try {
        ...
        // memeriksa adanya token
        if (token) {
          let res = await Axios.post(`${API_URL}/product/konversiStock`,{
